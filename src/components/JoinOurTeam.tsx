...
export default function JoinOurTeam() {
  return (
    <section id="career" className="py-24 max-w-7xl mx-auto px-6 lg:px-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {roles.map((role, idx) => (
          <div key={idx} className="bg-[#FDFBF7]/85 backdrop-blur-md rounded-[32px] p-8 lg:p-10 text-center flex flex-col items-center hover:shadow-2xl transition-all duration-300 border border-[#d4a373]/20">
            <div className="w-16 h-16 rounded-full bg-[#110d0a]/5 flex items-center justify-center text-[#d4a373] mb-6">
              <role.icon size={32} strokeWidth={1.5} />
            </div>
            <h3 className="text-2xl text-[#110d0a] font-serif mb-4">{role.title}</h3>
            <p className="text-[#5a4a42] leading-relaxed mb-10 flex-grow">{role.desc}</p>
